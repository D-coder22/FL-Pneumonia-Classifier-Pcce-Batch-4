...
from classifiers.constants import GENERATE_CONFIG_DESC, NUM_PARTIES_DESC, \
...
def check_valid_folder_structure(p):
    ...
    for folder in FL_EXAMPLES:
        if not os.path.isfile(os.path.join("classifiers", folder, "generate_configs.py")):
            p.error("Bad folder structure: '{}' directory is missing files.".format(folder))
...
if __name__ == '__main__':
    ...
    folder_configs = os.path.join("classifiers", "configs")
    ...
    config_model = import_module('classifiers.{}.generate_configs'.format(model))
